...
class User
{
    public function IdentityMap(string $domainObjectName, int $objectId = null, string $login = null)
    {
        ...
        if (isset($objectId)) {
            $domainObject = $this->dataProvider->getDataFromSource(['id' => $objectId]);
        } else {
            $domainObject = $this->dataProvider->getDataFromSource(['login' => $login]);
        }
    ...
    {
        $getUser = $this->IdentityMap('User', $id);
    ...
    {
        $getUser = $this->IdentityMap('User', $login);
